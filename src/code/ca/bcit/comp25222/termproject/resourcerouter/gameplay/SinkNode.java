...
import ca.bcit.comp25222.termproject.resourcerouter.util.ResourceType;
...
public final class SinkNode extends GameNode
{
                    ...
                    final Map<ResourceType, Integer> demandMap)
    {
        ...
        if (demandMap.isEmpty() ||
            demandMap.values().stream().anyMatch(c -> c <= DEFAULT_VALUE))
        {
    ...
    private Polygon createOctagon()
    {
        ...
        for (int i = 0; i < OCTAGON_SIDES; i++)
        {
            ...
            vx = center + SinkNode.RADIUS * Math.cos(angleRad);
            vy = center + SinkNode.RADIUS * Math.sin(angleRad);
    ...
    private void updateFill()
    {
        ...
        else
        {
            ...
            {
                final double offset;
                offset = (double) i / (n - OFFSET_CALCULATOR);

                stops[i] = new Stop(offset, colors.get(i));
            }
            final LinearGradient gradient;
            gradient = new LinearGradient(DEFAULT_VALUE,
                                          DEFAULT_VALUE,
                                          OFFSET_CALCULATOR,
                                          OFFSET_CALCULATOR,
                                          true,
    ...
    {
        final boolean isSat;
        isSat = currentDemand.values().stream().allMatch(c -> c <= DEFAULT_VALUE);
            ...
            final GameController gc)
    {
        ...
        for (final Pipe p : new ArrayList<>(incomingPipes))
        {
            ...
            if (r != null)
            {
                if (currentDemand.containsKey(r) &&
                    currentDemand.get(r) > DEFAULT_VALUE)
                {
                    p.clearResource();
                    gc.updatePipeVisual(p);
