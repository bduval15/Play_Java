             ...
             extends GameNode
{
    ...
    private static final double INITIAL_STROKE_WIDTH        = 0.5;
    ...
    private boolean consumeInputs(final GameController gc)
    {
        ...
        for (final Pipe p : new java.util.ArrayList<>(incomingPipes))
        {
            ...
            if (r != null)
            {
                ...
                current     = inputBuffer.getOrDefault(r, DEFAULT_VALUE);
                required    = inputRecipe.get(r);

                if (current + MIN_DELAY_TICKS > required)
                {
                    errorState = true;
                    updateVisualState();
                ...
                inputBuffer.merge(r, MIN_DELAY_TICKS, Integer::sum);
                p.clearResource();
